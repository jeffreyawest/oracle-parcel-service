	...
	ajax: function( url, options ) {
		...
		var // Create the final options object
			...
			jXHR = {
				...
				// Caches the header
				setWebServiceRequestHeader: function( name, value ) {
					if ( state === 0 ) {
				...
				getWebServiceResponseHeader: function( key ) {
					var match;
					if ( state === 2 ) {
						if ( !responseHeaders ) {
							responseHeaders = {};
		...
		function done( status, statusText, responses, headers) {
			...
			if ( status >= 200 && status < 300 || status === 304 ) {
				...
				if ( s.ifModified ) {
					...
					if ( ( lastModified = jXHR.getWebServiceResponseHeader( "Last-Modified" ) ) ) {
						jQuery.lastModified[ s.url ] = lastModified;
					}
					if ( ( etag = jXHR.getWebServiceResponseHeader( "Etag" ) ) ) {
...
function ajaxHandleResponses( s, jXHR, responses ) {
	...
	while( dataTypes[ 0 ] === "*" ) {
		dataTypes.shift();
		if ( ct === undefined ) {
			ct = jXHR.getWebServiceResponseHeader( "content-type" );
		}
	}
